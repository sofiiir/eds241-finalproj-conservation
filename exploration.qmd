...
data.frame(fitted = fitted(turtle_bpue_glm), residuals = residuals(turtle_bpue_glm)) %>% 
...
#| message: false
# Create residuals plot based on fitted values and residuals of glmm
data.frame(fitted = fitted(cpue_glm), residuals = residuals(cpue_glm)) %>% 
  ggplot(aes(x = fitted, y = residuals)) + geom_point(shape = 21) + 
  geom_hline(yintercept = 0, color = "red", linetype = "solid") + 
  theme_minimal() + labs(x = "Fitted Values",
         y = "Residuals",
...
Here, the log-transformed sea turtle BPUE and CPUE outcomes were plotted against treatment to verify the log-linear assumption, confirming that the relationship between the treatment and the ouctome is reasonable on the log scale.

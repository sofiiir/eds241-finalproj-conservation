...
A histogram of residuals is utilized to assess whether the residuals are approximately normally distributed and centered around zero. This visualization is critical to check whether the model adequately absorbs the non-normal structure of the BPUE and CPUE data and that no systematic patterns remain unexplained.

For GLMMs, raw residuals are not expected to follow a normal distribution, particularly when using non-Gaussian families such as Tweedie. Ideally, [model diagnostics are instead performed using simulated quantile residuals](https://stats.stackexchange.com/questions/295340/what-to-do-with-glm-gamma-when-residuals-are-not-normally-distributed) from the [DHARma](https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html) package, as quantile residuals are specifically designed to follow a normal distribution. In this analysis, however, DHARma residuals could not be computed directly due to package constraints. As a result, this cavaet should be kept in mind when considering the normality of residuals assumption. Instead, a histogram
...
bpue_resid <- residuals(turtle_bpue_glm)

# Plot
data.frame(residuals = bpue_resid) %>%
  ggplot(aes(x = residuals)) +
  geom_histogram(bins = 30, fill = "#39f72e", color = "white") +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  theme_minimal() +
  labs(title = "Histogram of Residuals for Turtle BPUE GLMM",
       x = "Residuals",
       y = "Count") +
  theme(panel.background = element_rect(fill = "#2d2f33"),
        plot.background = element_rect(fill = "#2d2f33"),
...
# Extract residuals from model
cpue_resid <- residuals(cpue_glm)

data.frame(residuals = cpue_resid) %>%
  ggplot(aes(x = residuals)) +
  geom_histogram(bins = 30, fill = "#39f72e", color = "white") +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  theme_minimal() +
  labs(title = "Histogram of Residuals for CPUE GLMM",
       x = "Residuals",
       y = "Count") +
  theme(panel.background = element_rect(fill = "#2d2f33"),
        plot.background = element_rect(fill = "#2d2f33"),
...
The histogram of residuals for the CPUE GLMM illustrate a left-skewed distribution, indicating a violation of the normality of residuals assumption. This pattern is consistent with the systematic pattern observed in the CPUE residuals vs fitted plot, suggesting that the Tweedie GLMM did not fully capture the zero-inflated structure of the CPUE data.
...
The Tweedie produced the lowest AIC for the BPUE model, confirming it as the better-fitting distribution. However, the Gamma distribution resulted in the lower AIC score compared to a Tweedie distribution for the CPUE model, suggesting that Tweedie does not provide a best fit to the data for this model.

...
AIC (Akaike Information Criterion) was used to compare model fit across distributions while accounting for model complexity, with lower AIC values indicating a better balance between goodness of fit and parsimony. A Gamma distribution was chosen as the comparison because like the Tweedi distribution, it handles right-skewed positive data within the same GLM framework. However, the Gamma is undefined at zero, so a slight adjustment of .001 was added before fitting. The Tweedie produced the lowest AIC for both the BPUE and CPUE models, confirming it as the better-fitting distribution. The Tweedie distribution had the lowest AIC for both models compared to a Gaussian distribution, indicating it provided the best fit to the data.

```{r}
## AIC comparison 
# Looking for lowest score to determine best distribution
bpue_gamma <- gam(bpue + .001 ~ treatment + s(gillnet_pair, bs = 're'),
                  family = Gamma(link = "log"),
                  data = bpue)

## AIC comparison 
# Looking for lowest score to determine best distribution
cpue_gamma <- gam(cpue + .001 ~ treatment + s(gillnet_pair, bs = 're'), 
                     family = Gamma(link = "log"),
                     data = cpue)
```
